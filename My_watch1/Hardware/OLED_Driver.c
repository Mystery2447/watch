...
{
    uint8_t tmp[2]={0x00,Command};
    HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, tmp, 2, 500);
}

void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    HAL_I2C_Mem_Write(&hi2c1,OLED_I2C_ADDR,0x40,I2C_MEMADD_SIZE_8BIT,Data,Count,500);
    //HAL_I2C_Master_Transmit(&hi2c1, OLED_I2C_ADDR, tmp, (Count+1), 5);
}
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowChar(int8_t X, int8_t Y, char Char, uint8_t FontSize)
{
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowASCII(int8_t X, int8_t Y, char *String, uint8_t FontSize)
{
	uint8_t i;
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowString(int8_t X, int8_t Y, char *String, uint8_t FontSize)	//中英文打印;
{
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowNum(int8_t X, int8_t Y, uint32_t Number, uint8_t Length, uint8_t FontSize)
{
	uint8_t i;
    if (X > 127){return;}
	if (Y > 64){return;}
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_ShowImage(int8_t X, int8_t Y0, uint8_t Width, uint8_t Height, const uint8_t *Image)
{
...
/**
  ...
  * 说    明：调用此函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_DrawRectangle(int8_t X, int8_t Y, uint8_t Width, uint8_t Height, uint8_t IsFilled)
{
	uint8_t i, j;
	if (!IsFilled)		//指定矩形不填充
	{
...
// uint8_t liangdu_UP=0x00,GUODUsn=0,GUODUspeed=11; //上次亮度，标志位，执行速率
// #include "Delay.h"  

// void OLED_transition(uint8_t o)  //屏幕缓灭缓亮，根据其传入的值来决定，使用前需要先激活其标志位GUODUsn，缓灭时为阻塞型函数
// {
// 	if(GUODUsn)
// 	{
// 			if(o)
